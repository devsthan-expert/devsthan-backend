...
const createTour = async (req, res) => {
  ...
  try {
    ...
    if (tour) {
      ...
      tour.meals=meals,
      tour.isStandard=isStandard,
      tour.isDeluxe=isDeluxe,
      tour.isPremium=isPremium,
      tour.activities=activities,
      
      tour.hotel=hotel,
      tour.metaTitle=metaTitle,
      tour.metaDescription=metaDescription,
      ...
      tour.knowBeforeYouGo = knowBeforeYouGo
...
const getAllTours = async (req, res) => {
  try {
    ...
    // Check if location is "allTours"
    if (location && location.toLowerCase() !== "alltours") {
    ...
    if (minPrice != null && maxPrice != null) {
      filteredTours = filteredTours.filter((tour) => {
        const tourPrice = parseFloat(tour.standardDetails?.pricing?.[0]?.price || 0);
        return (
          !isNaN(tourPrice) &&
...
const getSelectedDetails = async (req, res) => {
    ...
    try {
      ...
      const tours = await Tour.find({}, "name standardDetails.pricing premiumDetails.pricing deluxeDetails.pricing country city state duration uuid isStandard isDeluxe isPremium bannerImage location");
